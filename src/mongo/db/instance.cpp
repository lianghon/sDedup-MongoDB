...
namespace mongo {
    ...
    typedef boost::unordered_map<unsigned long long, long long> Ts2CursorId; 
    Ts2CursorId cursorMap;

    // lock for getMore
    boost::mutex mutexGetMore;
...
namespace {
    ...
    void receivedUpdate(OperationContext* txn,
        ...
        /*LX: restore deduped data for update object*/
        /*
        BSONObj toupdate;
        BSONObj dedupToupdate = d.nextJsObj();
        pdedup->restoreBSON(dedupToupdate, toupdate);
        */
    ...
    bool receivedGetMore(OperationContext* txn,
        ...
        while( 1 ) {
            ...
            //log() << "LX: receivedGetMore: No AssertionException.";
        ...
        Message *resp = new Message();
        //int resultFlags = msgdata.getResultFlags();
        //log() << "LX: receivedGetMore: resultFlags: " << resultFlags;

        //if (resultFlags & ResultFlag_Dedup) {
        if (0) {

            // Lock so that no other secondary nodes can sync at the same time
            // This is needed for deduplication to be done safely
            boost::mutex::scoped_lock lk(mutexGetMore);
            // LOG(0) << "LX: receivedGetMore: Dedup enabled: resultFlags: " << resultFlags << " cursorid: " << cursorid << " nReturned: " << msgdata.getNReturned();
            ...
            bb.skip(sizeof(QueryResult::Value));
            char *end = msgdata.msgdata().view2ptr() + msgdata.msgdata().getLen();
            const char *nextobj = msgdata.msgdata().view2ptr() + sizeof(QueryResult::Value);

            while (nextobj < end) {
                obj = BSONObj(nextobj);
                // log() << "LX: receivedGetMore: get next obj" << obj.jsonString(Strict, 1);
                
                // Currently, only dedup "insert"s
                if (obj["op"].String().compare("i") != 0) {
                    bb.appendBuf((void*)obj.objdata(), obj.objsize());
                    nextobj += obj.objsize();
                    break;
                } 
                
                BSONObjBuilder bbld;    // bb.skip(4);
                BSONObjIterator bi(obj);
                BSONElement e;
                std::string objNs;
                // Append all fields except "o"
                while ( bi.more() ) {
                    e = bi.next();
                ...
                BSONObj dedupedObj;
                pdedup->dedupBSON(objNs, subObj, dedupedObj);
                bbld.append("o", dedupedObj);
                BSONObj newObj = bbld.obj();
                //log() << "LX: receivedGetMore: newObj: " << newObj.jsonString(Strict, 1);
                bb.appendBuf((void*)newObj.objdata(), newObj.objsize());
                
                
                /*
                // re-build the bsonobj with deduplicated data
                // Add result to output buffer.
                // int objOffset = bb.len();
                BSONObjBuilder bbld(bb);    // bb.skip(4);
                BSONObjIterator bi(obj);
                BSONElement e;
                std::string ns;
                // Append all fields except "o"
                ...
                while ( bi.more() ) {
                    ...
                    if (strcmp(e.fieldName(), "ns") == 0) 
                        ns = e.String();
                    //log() << "element string: " << e.jsonString(Strict, 1, 1);
                    if (strcmp(e.fieldName(), "o") == 0) 
                        break;
                    bbld.append(e);
                }
                ...
                // buffer offset of the new object
                //int objBufOffset = bb.len();

                //BSONObjIterator subBi(subObj);
                // get "_id" element
                //BSONElement subE = subBi.next();
                //subBbld.append(subE);
                //int srcLen = subObj.objsize() - 5 - subE.size();
                BSONObj subObj = e.Obj();
                BSONElement oidE = subObj["_id"];
                mongo::OID oid = oidE.OID();
                

                mongo::dedup::DiskLoc dstLoc(ns, oid), srcLoc;
                std::vector<mongo::dedup::Segment> matchSeg;
                std::vector<unsigned char> unqData;
                // add deduplicated "o" to buf

                // whether or not to add index for this blob
                // NOTE: for now, we assume there are exactly 2 secondaries.
                

                // OpTime optime = obj["ts"]._opTime();
                // Ts2CursorId::iterator it = cursorMap.find(optime.asDate());
                // bool setIndex = false;
                // if ( it == cursorMap.end() ) {
                //     cursorMap[optime.asDate()] = cursorid;
                // }
                // else {
                //     long long cId = (*it).second;
                //     if (cursorid != cId) {
                //         setIndex = true;
                //         cursorMap.erase(optime.asDate());
                //     }
                // }
                  
                bool setIndex = true;

                //log() << "LX: Dedup start. OID: " << oid.toString();
                Timer *dTimer = new Timer();
                long long dedupMicros = 0;
                int ret = pdedup->processBlob(
                    subObj, dstLoc, srcLoc, matchSeg, unqData, setIndex);
                dedupMicros += dTimer->micros();
                delete dTimer;
                //log() << "LX: Dedup latency: " << dedupMicros << "    " 
                //    << "doc length: " << dstLen;
                ...
                //log() << "LX: getMore: processBlob returns code: " << ret << " setIndex: " << (int) setIndex << " cursorId: " << cursorid;
                //log() << "dstLen: " << dstLen << " unqData Len: " << unqData.size() << " matchSeg size: " << matchSeg.size();
                
                if (ret == 0) {
                    //LOG(0) << "LX: Whole blob duplicate: srcLoc oid: " << srcLoc.oid.toString();
                    // whole blob duplicate
                    // Dedup type, source blob OID, source namespace
                    
                    // build new "o" object in oplog entry

                    bb.appendNum((char) Object);
                    bb.appendStr("o");
                    BSONObjBuilder subBbld(bb);
                    subBbld.append(oidE);
                    int binLen = 1 + 12 + srcNs.size() + 1;
                    ...
                    BSONObjBuilder subBbld(bb);
                    subBbld.append(oidE);
                    int binLen = 1 + 12 +  (srcNs.size() + 1) + sizeof(int) + matchSeg.size() * 
                ...
                */
                ...
                /*
                BSONObj newObj(bb.buf() + objBufOffset);
                log() << "newObj size: " << newObj.objsize();
                log() << "newObj string: " << newObj.jsonString(Strict, 1);
                */
                nextobj += obj.objsize();
            }

            // QueryResult* qr = reinterpret_cast<QueryResult*>(bb.buf());
            // memcpy((void *)qr, (void *)msgdata, sizeof(QueryResult));
            // qr->len = bb.len();
            // bb.decouple();

            QueryResult::View qr = bb.buf();
            qr.msgdata().setLen(bb.len());
            qr.msgdata().setOperation(msgdata.msgdata().getOperation());
            qr.setResultFlags(msgdata.getResultFlags());
            qr.setCursorId(msgdata.getCursorId());
            qr.setStartingFrom(msgdata.getStartingFrom());
            qr.setNReturned(msgdata.getNReturned());
            bb.decouple();

            resp->setData(qr.view2ptr(), true);
        }
        
        // no ResultFlag_Dedup in resultFlags
        else {
    ...
    void checkAndInsert(OperationContext* txn,
        ...
        while ( true ) {
            try {
                ...
                //LOG(0) << "LX: checkAndInsert: dedup before insertion";
                BSONObj newObj;
                pdedup->dedupBSON(ns, js, newObj);
                long dedup_tm = timer->micros();
                timer->reset();
                LOG(0) << "LX: ORIGINAL obj string: " << js.jsonString(Strict, 1);
                LOG(0) << "LX: checkAndInsert: NEW obj string: " << newObj.jsonString(Strict, 1);
                StatusWith<RecordId> status = collection->insertDocument( txn, newObj, true );
                long insert_tm = timer-> micros();
                if (!js["sha1"].eoo()) {
                    double compRatio = 1.0 * js.objsize() / newObj.objsize();
                    //LOG(0) << "LX: CheckAndInsert: " << js["sha1"].toString(false) << ". insert time: " << insert_tm << " micros.";
                    LOG(0) << "LX: CheckAndInsert: " << js["sha1"].toString(false) << ". dedup time: " << dedup_tm << " micros. insert time: " << insert_tm << " micros.";
                    LOG(0) << "LX: CheckAndInsert: objsize: " << js.objsize() << ". comp ratio: " << compRatio << ". dedup time: " << dedup_tm;
                }
    ...
    void checkAndInsert(OperationContext* txn,
                        ...
                        BSONObj& dedupObj) {
        BSONObj js;
        //log() << "LX: checkAndInsert: ns: " << std::string(ns);
        //log() << "LX: checkAndInsert: dedupObj: " << dedupObj.toString();
        pdedup->restoreBSON(dedupObj, js);
        //log() << "LX: checkAndInsert: js: " << js.toString();
        ...
        //log() << "LX: checkAndInsert: insert to database.";
        StatusWith<DiskLoc> status = collection->insertDocument( txn, js, true );
        uassertStatusOK( status.getStatus() );
        //repl::logOp(txn, "i", ns, js);
        repl::logOp(txn, "i", ns, dedupObj);
        //log() << "LX: checkAndInsert: append to oplog.";
        //log() << dedupObj.toString();
